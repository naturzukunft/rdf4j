...
public class SparqlQueryEvaluatorDefaultTest {
    ...
    @Test
    public void queryWithDefaultGraphUriAndNamedGraphUris() throws MalformedQueryException, IllegalStateException, IOException {
        Repository repo = new SailRepository(new MemoryStore());
        try (RepositoryConnection con = repo.getConnection()) {
            con.add(getTestModel1(), CTX1);
            con.add(getTestModel2(), CTX2);
    ...
    private ArrayNode asArrayNode(EvaluateResult evaluateResult) throws IOException, JsonProcessingException, JsonMappingException {
